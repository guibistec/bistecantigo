...
// constructor
	...
	<p><label for="<?php echo $this->get_field_id( 'phone2' ); ?>"><?php _e( 'Enter your technical support phone number', 'moesia' ); ?></label>
	...
	function widget($args, $instance) {
		...
		$address   = isset( $instance['address'] ) ? esc_html( $instance['address'] ) : '';
		$phone   = isset( $instance['phone'] ) ? esc_html( $instance['phone'] ) : '';
		$phone   = isset( $instance['phone2'] ) ? esc_html( $instance['phone2'] ) : '';
		$email   = isset( $instance['email'] ) ? esc_html( $instance['email'] ) : '';
		...
		if( ($phone) ) {
			...
			echo '<span><i class="fa fa-phone"></i>' . __('Telefone: ', 'moesia') . '</span>' . $phone;
			echo '</div>';
		}
		if( ($phone2) ) {
			echo '<div class="contact-phone">';
			echo '<span><i class="fa fa-cog"></i>' . __('Suporte Técnico: ', 'moesia') . '</span>' . $phone;
			echo '</div>';
		}
		if( ($email) ) {
